...
export default function CreateProjectPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (mediaFiles.length > 0 && res.data?.data?.id) {
        const mediaFormData = new FormData();
        mediaFiles.forEach(f => mediaFormData.append('files', f));
        mediaFormData.append('media_type', 'image');
        await projectsAPI.addMedia(res.data.data.id, mediaFormData);
      }
    ...
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="dark:text-gray-400 text-gray-500 text-sm">Create a new portfolio project</p>
        </div>
      </div>
      ...
      <div className="glass-card p-6 sm:p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center gap-3 p-4 bg-[var(--bg-muted)] rounded-xl border border-[var(--border-color)]">
            ...
            <label htmlFor="is_published" className="text-sm font-medium cursor-pointer">Publish this project immediately</label>
          </div>
          ...
          <div>
            ...
            <div {...mediaDropzone.getRootProps()}
              ...
              <p className="text-sm dark:text-gray-400 text-gray-500">Drag & drop images/videos for gallery</p>
            </div>
            {mediaPreviews.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
                {mediaPreviews.map((p, i) => (
                  <div key={i} className="relative group aspect-video">
                    <img src={p} alt="" className="w-full h-full object-cover rounded-lg border border-[var(--border-color)]" />
          ...
          <div className="flex gap-4 pt-6 border-t border-[var(--border-color)]">
            <Link href="/admin/projects" className="btn-secondary px-6">{t('common.cancel')}</Link>
